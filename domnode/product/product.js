var data = {
  ...
  const changeRange =()=>{
    ...
    var filProducts = data.products.filter((prod)=>{
        return prod.price<=range.value;
    })
    for(let i=0;i<filProducts.length;i++){

        count++;
        const h1 = document.createElement("h1")
        h1.innerHTML = count +" "+ filProducts[i].title + " " + filProducts[i].price
        root.appendChild(h1)
    }
...
const searchProd=()=>{
    ...
    // const filProduct = data.products.filter((prod)=>{
    //     return prod.title.includes(productName.value)
    // })
    // ... spread operator
    const filProducts = data.products.map((prod)=>{
        //return prod.title.toLowerCase()
        return {
            ...prod,
            title:prod.title.toLowerCase()
        }
    }).filter((p)=>{
        return p.title.includes(productName.value)
    })
    for(let i=0;i<filProducts.length;i++){
